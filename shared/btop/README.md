...
Activo y enlazado desde `~/.config/btop` hacia `~/mydotfiles/shared/btop`.
...
```bash
ln -s ~/mydotfiles/shared/btop ~/.config/btop
```

En esta maquina la carpeta previa quedo respaldada en
`~/.config/btop.bak-before-mydotfiles`. La fuente de verdad queda en
`~/mydotfiles/shared/btop`.
