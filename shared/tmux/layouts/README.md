...
mkdir -p ~/mydotfiles/shared/tmux/layouts/$LAYOUT_NAME
tmux display-message -p '#{window_layout}' > ~/mydotfiles/shared/tmux/layouts/$LAYOUT_NAME/layout.txt
```
...
LAYOUT=$(cat ~/mydotfiles/shared/tmux/layouts/new-layout/layout.txt)
...
```bash
chmod +x ~/mydotfiles/shared/tmux/layouts/$LAYOUT_NAME/apply_layout.sh
```

Y agregar un bind en `tmux/tmux.conf`:

```tmux
bind L run-shell "~/mydotfiles/shared/tmux/layouts/new-layout/apply_layout.sh"
```
