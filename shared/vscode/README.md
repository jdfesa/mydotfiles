...
```bash
~/Library/Application Support/Code/User/settings.json -> ~/mydotfiles/shared/vscode/settings.json
```
...
```bash
cat ~/mydotfiles/shared/vscode/extensions.txt | xargs -L 1 code --install-extension
```
...
```jsonc
"custom-ui-style.external.imports": [
  "file:///Users/jd/mydotfiles/shared/vscode/custom.css"
],
...
```bash
cp ~/mydotfiles/shared/vscode/settings.json.bak ~/mydotfiles/shared/vscode/settings.json
```

Si quieres volver exactamente al estado anterior de este repo antes de esta prueba y todavia no hiciste commit:

```bash
git restore shared/vscode/settings.json shared/vscode/extensions.txt shared/vscode/README.md
rm ~/mydotfiles/shared/vscode/custom.css
```
