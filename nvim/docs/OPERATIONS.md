...
```bash
~/.local/share/nvim/mason/bin/stylua ~/mydotfiles/nvim/lua
```

Ver plugins instalados desde el lockfile sin depender de `jq`:

```bash
node -e 'const fs=require("fs"); const lock=JSON.parse(fs.readFileSync("nvim/lazy-lock.json","utf8")); console.log(Object.keys(lock).sort().join("\n"));'
```
...
Si aparece un warning sobre `es.utf-8.spl`, revisar `nvim/lua/config/options.lua`.
